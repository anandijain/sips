...
def remove_missed_wins(games):
    ...
    if isinstance(games, dict):
        for g_id in list(games.keys()):
            wins_sum = len(games[g_id]['a_win'].unique()) + \
                len(games[g_id]['h_win'].unique())
            if wins_sum != 3:
                del games[g_id]
...
def drop_null_times(df, columns=['lms_date', 'lms_time']):
    '''
    given df and list of columns. convert '0' values to np.datetime64
    '''
    init_len = len(df)
    for col in columns:
        ...
        print('len(df) before: {}, after length: {}, delta: {}'.format(
            init_len, after_len, delta))
    return df
...
def dates(df):
    ...
    date_categories = [dt.year, dt.month, dt.week, dt.day,
                       ...
                       dt.dayofyear]
    col_names = ['year', 'month', 'week', 'day', 'hour',
...
def onehot_teams(a_team, h_team):
    # take in 2 teams playing and onehot to alphabetically, returns series
    one_hotted = []
