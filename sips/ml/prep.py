...
def main(train_frac=0.7):
    to_nums = ['secs', 'live', 'a_pts', 'h_pts', 'a_ps', 'h_ps', 'a_hcap', 'h_hcap',
               'a_ml', 'h_ml', 'a_tot', 'h_tot', 'a_hcap_tot', 'h_hcap_tot']
    ml_cols = ['a_hcap', 'h_hcap', 'a_ml', 'h_ml', 'a_hcap_tot', 'h_hcap_tot']
    to_dummies = ['a_team', 'h_team', 'quarter',
                  'status', 'game_id']  # not including teams
    dfs = helpers.get_dfs()
    df = pd.concat(dfs)
    df = clean(df)
